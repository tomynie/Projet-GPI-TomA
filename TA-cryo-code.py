...
def extract_particles(img_final, pics_orientes, carte_max_ncc, carte_angles,
    ...
    # grille de toutes les particules — moins de colonnes et plus grande taille
    cols = min(4, n)
    rows = (n + cols - 1) // cols
    ...
    for idx in range(rows * cols):
        ...
        if idx < n:
            ...
            titre = f"#{idx + 1}  |  NCC : {scores_extraits[idx]:.2f}  |  {angles_extraits[idx]}°"
            couleur = "gold" if idx == idx_best else "white"
            ax.set_title(titre, fontsize=9, color=couleur)
        ax.axis("off")
    plt.suptitle(f"Particules extraites — {n} au total  (or = meilleure)", fontsize=13)
    plt.tight_layout()
