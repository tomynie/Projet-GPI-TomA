...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Chemin vers le fichier MRC. Si absent, telecharge depuis Google Drive."
    )
    parser.add_argument(
...
def load_mrc(mrc_path, results_dir):
    separator("ETAPE 1 : Chargement de la micrographie")

    # si pas de fichier fourni, on telecharge depuis Google Drive
    if mrc_path is None:
        ...
        if not os.path.exists(mrc_path):
            print("Aucun fichier MRC fourni -> telechargement depuis Google Drive...")
            gdown.download(id=file_id, output=mrc_path, quiet=False, fuzzy=True)
        else:
            print(f"Fichier MRC deja present : {mrc_path}")
    else:
        print(f"Fichier MRC : {mrc_path}")
...
def preprocess(image_2d, bin_factor, results_dir):
    ...
    # le binning reduit la taille de l image et augmente le pixel size
    print(f"Binning x{bin_factor} : {image_2d.shape} -> ", end="")
    img_binned = transform.downscale_local_mean(image_2d, (bin_factor, bin_factor))
    print(f"{img_binned.shape}")
    print(f"Pixel size : 0.66 x {bin_factor} = {0.66 * bin_factor:.2f} A/px")

    separator("ETAPE 1.2 : Normalisation Z-score")

    # Z-score indispensable pour que la NCC soit bien calibree
    img_norm = (img_binned - img_binned.mean()) / img_binned.std()
    print(f"Apres normalisation : moyenne={img_norm.mean():.3f}, std={img_norm.std():.3f}")

    separator("ETAPE 1.3 : Padding")

    # bordure de zeros pour eviter les effets de bord lors du glissement du template
    pad_size = 100
    img_padded = np.pad(img_norm, pad_width=pad_size, mode="constant", constant_values=0)
    print(f"Padding {pad_size}px : {img_norm.shape} -> {img_padded.shape}")
...
def create_template(structure, pixel_size, results_dir):
    ...
    print(f"Box size : {box_size} px ({box_size * pixel_size:.1f} A) | pixel size : {pixel_size} A/px")
...
def run_angular_search(img_final, template_pour_ncc, threshold, results_dir):
    ...
    print(f"Recherche angulaire : {len(angles)} angles...")
    for i, angle in enumerate(angles):
        print(f"  angle {angle:3d} deg ({i+1}/{len(angles)})", end="\r")
    ...
    if len(pics_orientes) > 0:
        ...
        print(f"Score NCC max        : {np.max(scores):.3f}")
        print(f"Score NCC min        : {np.min(scores):.3f}")
...
def extract_particles(img_final, pics_orientes, carte_max_ncc,
    ...
    particules = []
    scores_extraits = []
    for (y, x), score in zip(pics_orientes, scores_finaux):
    ...
    # grille de toutes les particules
    cols = min(8, n)
    rows = (n + cols - 1) // cols
    ...
    for idx in range(rows * cols):
        ...
        ax.axis("off")
    plt.suptitle(f"Particules extraites - {n} au total", fontsize=12)
    plt.tight_layout()
...
def main():
    ...
    # etape 1
    image_2d = load_mrc(args.mrc, results_dir)
    ...
    # etape 4
    extract_particles(
        img_final, pics_orientes, carte_max_ncc,
        template_final, template_pour_ncc, results_dir
    )
